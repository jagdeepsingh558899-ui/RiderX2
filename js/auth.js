/* ============================================================
   ...
   Firestore /users/{uid} = profile + role
...
function normalizeRole(value) {
    const role = safeLower(value).replace(/[\s-]+/g, "_");
...
async function getUserProfile(user) {
    ...
    const snapshot = await FB.getDoc(
        getUserRef(user.uid)
    );
...
async function createUserProfile(
    ...
    if (normalizedRole === ROLES.RIDER) {
        ...
        profile.approvalStatus =
            safeString(extra.approvalStatus) ||
            "pending";
    }
...
function createRoleMismatchError(
    ...
    error.code = "riderx/role-mismatch";
...
function cacheAuthenticatedSession(
    ...
    if (normalizedRole === ROLES.CUSTOMER) {
        ...
        storageRemove(STORAGE.riderId);
        storageRemove(STORAGE.rider);
    }
    ...
    if (normalizedRole === ROLES.RIDER) {
        ...
        storageRemove(STORAGE.customerId);
        storageRemove(STORAGE.customer);
    }
...
async function finalizeAuthenticatedUser(
    ...
    /* --------------------------------------------------------
       PROFILE MISSING
       Only registration/explicit recovery may create it.
    -------------------------------------------------------- */
...
/* ============================================================
   DELETE NEW AUTH ACCOUNT ON ROLLBACK
============================================================ */
...
async function registerWithEmail(options = {}) {
    ...
    if (
        name &&
        typeof FB.updateProfile === "function"
    ) {
        try {
...
async function loginWithEmail(options = {}) {
    ...
    /*
     * IMPORTANT:
     * The selected role is only a validation hint.
     * Firebase + Firestore remain the real source of truth.
     */

    const selectedRole = normalizeRole(
        options.role ||
        options.selectedRole ||
        storageGet(STORAGE.selectedRole)
    );
    ...
    if (
        NORMAL_USER_ROLES.includes(selectedRole)
    ) {
        storageSet(
            STORAGE.selectedRole,
            selectedRole
        ...
        result =
            await finalizeAuthenticatedUser(
                user,
                {
                    ...
                    /*
                     * NEVER create a profile during login.
                     * This prevents wrong role/account creation.
                     */
    ...
    /*
     * At this point role is ALWAYS taken from Firestore.
     * This is what prevents Customer login from being treated
     * as Rider or vice versa.
     */
...
async function verifyPhoneOtp(options = {}) {
    ...
    if (!profile) {
        ...
        if (
            NORMAL_USER_ROLES.includes(existingRole) &&
            ...
            FB.onAuthStateChanged(
                ...
                async user => {
                    try {
                         ...
                         * SIGNED IN
                         *
                         * Always read Firestore profile.
                         * Never trust localStorage role.
                         */
                        let result;
...
function getDefaultRouteForRole(role) {
    ...
    if (normalized === ROLES.CUSTOMER) {
        return ROUTES.customerHome;
    }

    if (normalized === ROLES.RIDER) {
...
async function requireAuth(options = {}) {
    ...
    if (options.role) {
        ...
        if (
            !requiredRole ||
            currentState.role !== requiredRole
        ) {
            if (
                options.redirect !== false &&
                typeof window !== "undefined"
...
globalThis.RiderXAuth = {
    ...
    requireCustomer:
        options =>
            AUTH.requireCustomer(options),

    requireRider:
        options =>
            AUTH.requireRider(options),

    requireAdmin:
        options =>
            AUTH.requireAdmin(options),
